...
def export_charts(
    ...
    xlabels = [fmt_month(m) for m in months]
    ...
    for idx, (lbl, lc) in enumerate(legend_items):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_project = project.replace(" ", "_")
    filepath = reports_dir / f"DORA_chart_{safe_project}_{mode}_{timestamp}.png"
    ...
    async with httpx.AsyncClient() as client:
        ...
        if mode == "pipelines":
            ...
            if run_per_project:
                for pname, builds in project_builds.items():
                    ...
                    export_charts(org, pname, "pipelines", months, df, lt, cfr, mttr_result)
        ...
        if mode == "pullrequests":
            for proj in selected_projects:
                ...
                export_charts(org, pname, "pullrequests", months, df, lt, cfr, mttr_result)
                drill_down(df, lt, cfr, mttr_result, months)
